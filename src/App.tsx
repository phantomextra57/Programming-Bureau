  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-[#E0E0E0] flex flex-col antialiased selection:bg-blue-500/30 selection:text-white" dir="rtl">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-4 sm:p-6 pb-20">
        ...
        {/* Firebase Security Rules Error Alert */}
        {firestoreError && (
          <div id="firebase-rules-alert" className="bg-amber-950/40 border border-amber-500/30 text-amber-300 rounded-2xl p-5 mb-6 text-xs leading-relaxed font-sans shadow-xl">
            <div className="flex items-start gap-3">
              <Info className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
              <div className="flex-1 flex flex-col gap-1.5 text-right">
                <span className="font-bold text-sm text-white">تنبيه: قواعد الحماية في Firebase تمنع مزامنة البيانات الحية</span>
                <span>لقد كشفت المنصة أن قواعد حماية قاعدة البيانات (Firestore Security Rules) غير مفعلة أو مغلقة حالياً في لوحة تحكم Firebase الخاصة بك.</span>
                <span className="text-white/60">بسبب هذا، لن تظهر المنشورات الجديدة بين الحسابات أو الأجهزة المختلفة، وسيتم تشغيل وضع "المحاكاة المحلية" كبديل مؤقت.</span>
                <div className="mt-2 text-[11px] text-amber-200 bg-black/40 p-3 rounded-lg border border-amber-500/10 flex flex-col gap-1">
                  <span className="font-bold">💡 لحل هذه المشكلة وتمكين مشاركة المنشورات عبر الأجهزة:</span>
                  <span>1. اذهب إلى <a href="https://console.firebase.google.com/" target="_blank" rel="noreferrer" className="underline text-blue-400 hover:text-blue-300">موقع Firebase Console</a> ثم اختر مشروعك.</span>
                  <span>2. من القائمة الجانبية، اختر <strong>Firestore Database</strong> ثم اضغط على تبويب <strong>Rules</strong> في الأعلى.</span>
                  <span>3. امسح القواعد الحالية واكتب القواعد العامة للسماح بقراءة وكتابة البيانات بشكل آمن، ثم اضغط على <strong>Publish</strong>.</span>
                </div>
              </div>
            </div>
          </div>
